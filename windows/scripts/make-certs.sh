...
# Portabile: funziona su Linux e su Windows (Git Bash, con openssl nel PATH).
# Rilanciare se cambia l'IP della rete (lo fa firetraker in automatico).
set -e
...
# IP IPv4 della rete: Linux usa `ip`, Windows Git Bash usa `ipconfig`
if command -v ip >/dev/null 2>&1; then
  ...
  IP_LIST=$(ipconfig | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}' | grep -vE '^(127\.|0\.0\.0\.0|255\.)' | sort -u || true)
fi

# mantiene anche eventuali IP salvati prima
if [ -f ips.txt ]; then
